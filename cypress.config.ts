...
writeExcelFile(rows: { [key: string]: any }[]) {
  ...
  const headers = [
    'School', 'Schoolname', 'URN', 'Previous URN', 'Region', 'Local authority', 'School phase',
    'Religious character', 'Diocese', 'Number on roll (NOR)', 'Date of last inspection',
  ...
  const updatedSheet = xlsx.utils.json_to_sheet(cleanedRows, { header: headers });
