...
describe("Enrich school data from GIAS site", () => {
  ...
  it("Should search and extract school data from GIAS and Ofsted", () => {
    ...
    Cypress._.each(schoolList, (row) => {
      cy.visit("https://get-information-schools.service.gov.uk/", {
      ...
      cy.get(".gias-tabs", { timeout: 10000 }).then(($el) => {
        if ($el.find("a").text().includes("Location")) {
          // Check if 'Location' exists in the tab
      ...
      cy.get(".govuk-summary-list")
        .first()
        .within(() => {
          cy.contains("Local authority")
            .next()
            .then(($el) => {
              row["Local authority"] = $el.text().trim();
            });
          cy.contains("Phase of education")
            .next()
            .then(($el) => {
      ...
      cy.get("body").then(($body) => {
        const link = $body.find(
          'a:contains("Ofsted report (opens in new tab)")'
        );
                      ...
                      cy.get("body").then(($ofstedBody) => {
                        ...
                        if ($ofstedBody.find("time").length) {
                          cy.get("time")
                            .first()
                            .then(($t) => {
                              ofstedData["Date of last inspection"] = $t
                                .text()
                                .trim();
                            });
                      ...
                      cy.get("body").then(($ofstedBody) => {
                        ...
                        if ($ofstedBody.find(".subjudgements__rates").length) {
                          cy.get(".subjudgements__rates").within(() => {
                            cy.contains("Quality of education")
                              .next()
                              .then(($q) => {
                                ofstedData["Quality of education"] =
                                  $q.text().trim() || "Unknown";
                              });
                            cy.contains("Behaviour and attitudes")
                              .next()
                              .then(($b) => {
                                ofstedData["Behaviour and attitudes"] =
                                  $b.text().trim() || "Unknown";
                              });
                            cy.contains("Personal development")
                              .next()
                              .then(($p) => {
                                ofstedData["Personal development"] =
                                  $p.text().trim() || "Unknown";
                              });
                            cy.contains("Leadership and management")
                              .next()
                              .then(($l) => {
                                ofstedData["Leadership and management"] =
                                  $l.text().trim() || "Unknown";
                              });
                          });
  ...
  // AFTER BLOCK SHOULD GET TRIGGERED ON ORGANIC TEST PASS OR ORGANIC TEST FAIL - IF THE TEST IS STOPPED by MANUAL INTERVENTION IN THE TEST-RUNNER OR STOPPED WITH CTRL + Z OR CTRL + C ON THE COMMAND-LINE THIS BLOCK WILL NOT BE TRIGGERED
  after(() => {
